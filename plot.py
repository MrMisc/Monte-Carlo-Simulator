...
if PeriodCounter>2:
    ...
    plt.style.use("fivethirtyeight")
    if len(df)>5:
        sns.boxplot(x = df['Primaries'],y = np.log(df['Amount']+1))
    plt.title(f"Primary sources of loss(default)/gain for Period 1")
    plt.savefig(f"boxplot{PeriodCounter}.png")
    # plt.show()
    if len(df)>5:
        sns.violinplot(x = df['Primaries'],y = np.log(df['Amount']+1))
    plt.title(f"Primary sources of loss(default)/gain for Period 1")
